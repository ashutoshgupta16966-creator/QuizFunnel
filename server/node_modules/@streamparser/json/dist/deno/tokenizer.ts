...
function validateBufferSize(name: string, size: number | undefined): void {
  if (size === undefined) return;
  if (!Number.isInteger(size) || size < 0) {
    throw new TokenizerError(
      `Invalid "${name}": ${size}. Expected a non-negative integer.`,
    );
  }
}
...
function utf8SequenceLength(leadByte: number): number {
  if (leadByte >= 194 && leadByte <= 223) return 2;
  if (leadByte <= 239) return 3;
  return 4;
}
...
function multiByteRunEnd(buffer: Uint8Array, start: number): number {
  let j = start;
  while (j < buffer.length && buffer[j] >= 128) {
    const seqLength = utf8SequenceLength(buffer[j]);
    if (j + seqLength > buffer.length) break; // split across the chunk boundary
    j += seqLength;
  }
  return j;
}
...
export default class Tokenizer {
  ...
  // A raw (non-escaped) surrogate pair split across two separate string
  // `write()` calls: a lone trailing high surrogate held back from the end
  // of a string chunk, to be prepended to the next string chunk before
  // encoding. Encoding each chunk independently would otherwise replace the
  // unpaired surrogate with U+FFFD, since TextEncoder always produces
  // well-formed UTF-8 and lone surrogates have no valid UTF-8 encoding.
  private pendingStringSurrogate?: string;
  private encoder = new TextEncoder();
  private offset = -1;
  private streamByteLength = 0; // Total bytes consumed across all write() calls before the current one
  ...
  // them).
  private appendUnicodeCodeUnit(intVal: number): void {
  ...
  private startIncompleteChar(buffer: Uint8Array, start: number): void {
  ...
  public write(input: Iterable<number> | string): void {
    try {
      ...
      if (input instanceof Uint8Array) {
        ...
        buffer = Uint8Array.from(input as Iterable<number>);
      } else {
        throw new TypeError(
          "Unexpected type. The `write` function only accepts Iterables (e.g. Arrays, Sets, Generators), TypedArrays and Strings.",
        );
      }
      ...
      for (let i = 0; i < buffer.length; i += 1) {
            ...
            if (n >= charset.SPACE) {
              ...
              while (j < buffer.length) {
                ...
                if (
                  b < charset.SPACE ||
                  b >= 128 ||
                  b === charset.QUOTATION_MARK ||
                  b === charset.REVERSE_SOLIDUS
                )
                  break;
                j += 1;
              }
        ...
        throw new TokenizerError(
          `Unexpected "${String.fromCharCode(
            n,
          )}" at chunk position "${i}" (absolute position "${
            this.streamByteLength + i
          }") in state ${TokenizerStateToString(this.state)}`,
        );
      }
